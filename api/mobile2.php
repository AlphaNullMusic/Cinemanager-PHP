...
if ($stmt->rowCount() > 0) {
	...
	$feed['movies'] = get_movie_list_full2(($_REQUEST['data'] == 'upcoming')?'cs':'ns', ($_REQUEST['data'] == 'upcoming')?'tbc,ml.priority,ml.release_date,m.title':'ml.priority,m.title', 7, '%W %e %b', '%e %b', 100, 'today', $cinemaIds);
	...
	if ($limit>0) {
		...
		*/
		...
		if (mysql_num_rows($movie_list_res)>0) {
          ...
          $movies[$n]['sessions'] = json_encode($raw_data);
					...
					// 	// }
					// } else {
          //   // $movies[$n]['sessions'] = isset($raw_data['cinemas'][$cinema_ids[0]]['movies']) ? $raw_data['cinemas'][$cinema_ids[0]]['movies'][$movie_list_data['movie_id']]['sessions'] : NULL;
          //   $movies[$n]['sessions'] = json_encode($raw_data['cinemas']);
					// }
				}
				$n++;
			}
